...
require __DIR__ . '/_db.php'; // Pfad anpassen, je nach Location
...
<head>
  ...
  <style>
    ...
    .meta {
      ...
      margin:8px 0 14px; font-size:12px; flex-wrap:wrap;
    }
    .meta label { display:flex; gap:4px; align-items:center; }
    .meta input {
      padding:3px 4px; font-size:11px; border:1px solid #ccc; border-radius:3px;
    }
    table {
      width:100%; border-collapse:collapse; background:#fff; margin-bottom:10px;
    }
    th, td {
      border:1px solid #ddd; padding:6px 8px; font-size:12px; text-align:center;
    }
    ...
    .ok {
      background:#d4f6d4; color:#176317; font-weight:600;
    }
    .bad {
      background:#ffd6d6; color:#8b0000; font-weight:600;
    }
    .outlier td { background:#ffb3b3 !important; } /* Ist > 150% Soll */
    .perf-good { color:#176317; font-weight:600; }
    .perf-bad  { color:#8b0000; font-weight:600; }
    .btn {
      display:inline-block; padding:6px 10px; margin:5px 4px 12px 0;
      font-size:11px; border-radius:4px; border:none; cursor:pointer;
...
<!-- Sachnummern (wie zuvor, gekürzt hier; deine Liste einsetzen) -->
<datalist id="sachnummern">
  <option value="W1 0Z1 010 010">
  <!-- ... alle weiteren wie bisher ... -->
</datalist>

<script>
// Mitarbeiter aus PHP (aktive aus DB)
const EMPLOYEES = <?= json_encode($employees, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) ?>;

// Vorgabezeiten (Placeholder – hier eure echten Daten rein)
...
function updateSummary() {
  ...
  rows.forEach(row => {
    ...
    const maName = (maInput.value || '').trim();
    if (!maName) return;

    const istVal = parseFloat((row.querySelector('.ist')?.textContent || '').replace(',', '.'));
    const sollVal = parseFloat((row.querySelector('.soll')?.textContent || '').replace(',', '.'));
    const hasData = (!Number.isNaN(istVal) && istVal > 0) || (!Number.isNaN(sollVal) && sollVal > 0);
    if (!hasData) return;

    if (!stats[maName]) {
      stats[maName] = { count: 0, ist: 0, soll: 0 };
    }

    stats[maName].count += 1;
    if (!Number.isNaN(istVal))  stats[maName].ist  += istVal;
    if (!Number.isNaN(sollVal)) stats[maName].soll += sollVal;
  });
  ...
  Object.keys(stats).forEach(ma => {
    ...
    let perfClass = '';
    if (ist > 0 && soll > 0) {
      const leistung = (soll / ist) * 100; // >100% = schneller = grün
