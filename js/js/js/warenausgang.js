...
const API_WE = '/LKW/api/warenausgang_api.php';
const API_ATT = '/LKW/api/attachments_api.php';
...
function renderRowFromDB(row) {
  ...
  const cells = [
    ...
    row.sachnummer, row.brt_gew, row.gebucht, row.gebucht_von
  ];
  ...
  tr.dataset.dbid  = row.id;               // wichtig: DB-ID merken
...
(async function loadAll() {
  try {
    ...
    if (j.ok) {
      ...
      // 2) Initial nach Ausg.-Nr. (Spalte 1) aufsteigend sortieren
      // (erste TH muss data-type="number" haben)
      table.querySelector('thead th:nth-child(1)')?.click();
...
function applyPalletRunningBadges() {
  // Nur sichtbare Zeilen in Bildschirm-Reihenfolge
  const rows = [...tbody.querySelectorAll('tr')].filter(tr => tr.offsetParent !== null);

  // alte Paletten-Badges in Spalte 2 entfernen (nur unsere)
  rows.forEach(tr => tr.children[1]
    ?.querySelectorAll('.badge-count.pallet')
    .forEach(n => n.remove())
  );

  // Gruppen mit gleicher Ausg.-Nr. durchlaufen
  let i = 0;
  while (i < rows.length) {
    ...
    if (key !== '') {
      let running = 0;
      for (let k = i; k < j; k++) {
        running += toInt(getCellValue(rows[k], 10)); // Spalte "Behälter"

        const td = rows[k].children[1]; // Spalte 2 (Lieferschein)
        if (!td) continue;

        const b = document.createElement('span');
        b.className = 'badge-count pallet badge rounded-pill ms-1 ' +
                      (running > TRUCK_PALLET_LIMIT ? 'bg-danger' : 'bg-secondary');
        b.title = `Paletten kumuliert für Ausg.-Nr. ${getCellValue(rows[k],0).trim()}: ${running}/${TRUCK_PALLET_LIMIT}`;
        b.textContent = `${running}/${TRUCK_PALLET_LIMIT}`;
        td.appendChild(b);
      }
    }
    i = j;
...
(async () => {
  try {
    const res = await fetch('/LKW/api/stammdaten_api.php?type=sachnummer&action=list', { credentials:'same-origin' });
...
function duplicateRow(srcTr, { markAsDup = true } = {}) {
  ...
  const ausgang = getCellValue(srcTr, 0);         // gleich lassen
  ...
  for (let i=0;i<17;i++){
    ...
    if (i===0) v = ausgang;           // Ausg.-Nr. bleibt
...
function computeStats() {
  ...
  const sumBySach       = new Map();                                   // Sachnr. -> Summe Behälter
  const offeneSet       = new Set();                                   // offene ausgangsnummern
  const offeneList      = new Map();                                   // für Ausgabe
  const sumByLagergruppe= new Map(TARGET_GROUPS.map(g => [g, 0]));     // LG -> Summe Behälter
  const detailsByLG = new Map(
  TARGET_GROUPS.map(g => [g, { deliveries:new Set(), zus:0, brtGew:0, perAusgang:new Map() }])
);
  ...
  // ===== Warenausgänge pro Tag (distinct ausgangsnummern) + Δ zum Vortag =====
  const perDayMap = new Map(); // key: 'YYYY-MM-DD' -> Set<ausgangsnummer>

  rows.forEach((tr) => {
    const dtText = (getCellValue(tr, 3) || '').trim(); // Datum
    ...
    const dayKey = dtText; // bereits YYYY-MM-DD
  ...
  if (perDayBody) {
    ...
    if (perDayArr.length === 0) {
      ...
      perDayArr.forEach(([dayKey, cnt]) => {
        ...
        `;
        // Klick filtert Tabelle auf das Datum (freie Suche)
  ...
  // ===== Restliche Aggregationen (Sachnummern, offene Nr., LG-Details) =====
  rows.forEach((tr) => {
    ...
    const brtGew    = toInt(getCellValue(tr, 14));  // BRT-GEW (kg)
    ...
    if (!gebucht && ausgang) { offeneSet.add(ausgang); offeneList.set(ausgang, true); }

    if (sumByLagergruppe.has(lg)) {
      sumByLagergruppe.set(lg, (sumByLagergruppe.get(lg) || 0) + behaelter);

      const d = detailsByLG.get(lg);
      d.deliveries.add(ausgang);
      d.zus    += zus;
      d.brtGew += brtGew;

      let pe = d.perAusgang.get(ausgang);
     if (!pe) { pe = { rows:0, behaelter:0, zus:0, brtGew:0 }; d.perAusgang.set(ausgang, pe); }
      pe.rows      += 1;
  ...
  // ===== Top-Sachnummern (nach Summe Behälter) =====
  const TOP = (typeof TOP_LIMIT !== 'undefined') ? TOP_LIMIT : 10;
  ...
  // Bindings (optional, falls vorhanden)
  bindSachStatsInteractions?.();  // für klickbare Top-Sachnummern
  // markOverweightGroups?.();     // ← wenn du die 21-t-Prüfung separat implementiert hast
}
...
function markOverweightGroups() {
  ...
  rows.forEach(tr => {
    const nr = (getCellValue(tr, 0) || '').trim();
    const kg = toInt(getCellValue(tr, 14)); // BRT-GEW-Spalte
    if (!nr) return;
...
document.getElementById('statsGruppen')?.addEventListener('click', (ev) => {
  ...
  // per Ausgangsnummer aufbereiten
  ...
  col.innerHTML = `
    <div class="p-2">
      <div class="small mb-2">
        <strong>${grp}</strong> — Warenausgänge: <strong>${deliveriesCount}</strong>
...
// === Drilldown: Top-Sachnummern (Accordion) ==================================
function getCurrentRange() {
  const key = (document.getElementById('statsRange')?.value) || 'current';
  return getRangeByKey(key); // nutzt deine bestehende Helper-Funktion
}

function gatherRowsInRangeForSach(sach) {
  const { start, end } = getCurrentRange();
  const rows = [...document.querySelectorAll('#ausgangTable tbody tr')]
    .filter(tr => tr.dataset.saved === '1' || tr.dataset.mode === 'edit')
    .filter(tr => {
      const dt = parseISODate(getCellValue(tr, 3));
      if (start && (!dt || dt < start)) return false;
      if (end   && (!dt || dt >= end))  return false;
      return (getCellValue(tr, 13).trim() === sach);
    });
  return rows;
}

function toggleSachDetail(anchorTr, sach) {
  // schon offen? -> schließen
  ...
  // Daten einsammeln
  const rows = gatherRowsInRangeForSach(sach);
  const byAusg = new Map();
  let sumZus = 0, sumbrt_gew = 0;

  for (const tr of rows) {
    const ausg   = getCellValue(tr, 0).trim();
    const zus    = toInt(getCellValue(tr, 11));
    const brt_gew  = toInt(getCellValue(tr, 14));
    const datum  = getCellValue(tr, 3);
    sumZus   += zus;
    sumbrt_gew += brt_gew;

    const acc = byAusg.get(ausg) || { ausgang: ausg, count: 0, sumZus: 0, sumbrt_gew: 0, dates: new Set() };
    acc.count += 1;
    acc.sumZus += zus;
    acc.sumbrt_gew += brt_gew;
    if (datum) acc.dates.add(datum);
    byAusg.set(ausg, acc);
  }

  const deliveries = byAusg.size;

  const rowsHtml = [...byAusg.values()]
    ...
    .map(v => {
      ...
      return `<tr>
        ...
        <td class="text-end">${v.sumbrt_gew}</td>
      </tr>`;
    }).join('');
    ...
    <td colspan="2">
      <div class="p-2 bg-body-tertiary rounded border">
        <div class="d-flex flex-wrap gap-2 mb-2">
          <span class="badge text-bg-primary">Ausgänge: <strong>${deliveries}</strong></span>
          <span class="badge text-bg-secondary">Zus.-Beh.: <strong>${sumZus}</strong></span>
          <span class="badge text-bg-success">brt_gew: <strong>${sumbrt_gew}</strong></span>
        </div>
        <div class="table-responsive">
          <table class="table table-sm mb-0">
            <thead>
...
function bindSachStatsInteractions() {
  ...
  tb.querySelectorAll('tr').forEach(tr => {
    const sach = tr.cells?.[0]?.textContent?.trim();
    if (!sach || tr.dataset.boundSach === '1') return;
    tr.dataset.boundSach = '1';
    tr.style.cursor = 'pointer';
    tr.addEventListener('click', () => toggleSachDetail(tr, sach));
  });
}
  ...
  btnEdit.addEventListener("click", async (e) => {
    ...
    if (isEditing) {
      ...
      // Live-LG aus Sachnummer koppeln
...
async function exportGroupToXlsx(clickedTr) {
  ...
  const rows = groupRows.map(tr => ({
    ...
    qty:  Number(String(getCellValue(tr, 10)).replace(/\./g,'').replace(',','.')) || 0, // K -> Behälter/KLT
    noLabel: false
  }));
  ...
  window.open("/LKW/druck_wa.html", "_blank");
}
...
function reapplyGroups() {
  ...
  applyPalletRunningBadges();
}

/** Debounce (nur EINMAL im File definieren) */
...
async function renderAttachmentList(ausgangNr) {
  ...
  files.forEach((f, idx) => {
    ...
    item.querySelector(".act-delete")?.addEventListener("click", async () => {
      if (confirm(`„${f.filename}“ wirklich löschen?`)) {
        ...
        const tr = [...tbody.querySelectorAll("tr")].find(tr => getCellValue(tr,0).trim() === ausgangNr);
  ...
  if (keepUrl) {
    const hit = files.find(x => `/LKW/${x.path_rel}` === keepUrl);
    if (!hit) clearPreview();
  }
}
...
async function upsertRowToServer(tr) {
  ...
  brt_gew:       Number(getCellValue(tr,14))||0,   // <<< NEU
  ...
  try {
    ...
    if (tr.dataset.isDup === '1') {
      setTimeout(() => {
        ...
        // nächste Zeile gleicher Ausg.-Nr. mit LEEREM Lieferschein und SICHTBAR
