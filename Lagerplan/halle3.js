...
// ===============================
const API_ROW_LABELS = "/Lagerplan/lager_row_labels_api.php";
...
const API_STAMM = "/api/stammdaten_api.php"; // liefert sachnummer + lagergruppe
...
function renderLgSummaryFromPlan() {
  ...
  const totalEl = document.getElementById("lgSummaryTotal");

  if (!tbody) return;

  const slots = [...document.querySelectorAll(".palette-slot[data-sach]")];
  const sel = typeof getSelectedLGs === "function" ? getSelectedLGs() : null;
  ...
  for (const slot of slots) {
    ...
    const lg = String(__SACH_TO_LG__?.get(sach) || "UNBEKANNT").trim() || "UNBEKANNT";

    if (sel && !sel.has(lg)) continue;

    const menge = Math.max(1, parseInt(String(slot.dataset.menge || "1"), 10) || 1);
    ...
    const lgObj = byLg.get(lg);
    lgObj.paletten += 1;
    lgObj.stueck += menge;
    lgObj.slots.push(slot);

    if (!lgObj.sachMap.has(sach)) {
      lgObj.sachMap.set(sach, {
        sach,
        paletten: 0,
        stueck: 0,
        slots: []
    ...
    const sachObj = lgObj.sachMap.get(sach);
    sachObj.paletten += 1;
    sachObj.stueck += menge;
    sachObj.slots.push(slot);
  }

  const lgRows = [...byLg.values()].sort((a, b) => {
    return b.paletten - a.paletten || String(a.lg).localeCompare(String(b.lg), "de");
  });
  ...
  if (!lgRows.length) {
    tbody.innerHTML = `
      <tr>
        <td colspan="4" class="text-muted small py-3">
          Keine Lagergruppen im aktuell geladenen Plan gefunden.
        </td>
      </tr>
    `;

    if (totalEl) totalEl.textContent = "0 Paletten · 0 Stück · 0 Sachnummern";
    return;
  }

  const totalPaletten = lgRows.reduce((sum, x) => sum + x.paletten, 0);
  const totalStueck = lgRows.reduce((sum, x) => sum + x.stueck, 0);
  const totalSach = new Set(
    lgRows.flatMap(x => [...x.sachMap.keys()])
  ).size;

  for (const lgObj of lgRows) {
    const lgKey = safeDomKey(lgObj.lg);

    const mainTr = document.createElement("tr");
    mainTr.className = "lg-drill-main";
    mainTr.dataset.lg = lgObj.lg;
    mainTr.style.cursor = "pointer";

    mainTr.innerHTML = `
      <td class="fw-semibold">
        <span class="me-1" data-lg-icon="${escapeHtml(lgObj.lg)}">▸</span>
        ${escapeHtml(lgObj.lg)}
      </td>
      <td class="text-end fw-semibold">${formatInt(lgObj.paletten)}</td>
      <td class="text-end">${formatInt(lgObj.stueck)}</td>
      <td class="text-end">${formatInt(lgObj.sachMap.size)}</td>
    `;

    const detailTr = document.createElement("tr");
    detailTr.className = "lg-drill-detail d-none";
    detailTr.dataset.lgDetail = lgObj.lg;

    const sachRowsHtml = [...lgObj.sachMap.values()]
      .sort((a, b) => b.paletten - a.paletten || String(a.sach).localeCompare(String(b.sach), "de"))
      .map((sachObj) => {
        const sachKey = safeDomKey(lgObj.lg + "_" + sachObj.sach);

        return `
          <tr class="lg-sach-row"
              data-lg="${escapeHtml(lgObj.lg)}"
              data-sach="${escapeHtml(sachObj.sach)}"
              data-sach-key="${escapeHtml(sachKey)}"
              style="cursor:pointer;">
            <td class="fw-semibold ps-3">
              <span class="me-1" data-sach-icon="${escapeHtml(sachKey)}">▸</span>
              ${escapeHtml(sachObj.sach)}
            </td>
            <td class="text-end">${formatInt(sachObj.paletten)}</td>
            <td class="text-end">${formatInt(sachObj.stueck)}</td>
          </tr>

          <tr class="lg-pos-row d-none" data-sach-detail="${escapeHtml(sachKey)}">
            <td colspan="3">
              <div class="small text-muted mb-2">
                Positionen für <strong>${escapeHtml(sachObj.sach)}</strong>
              </div>
              <div class="lg-position-list">
                ${buildLgPositionButtonsHtml(sachObj.slots)}
              </div>
            </td>
          </tr>
        `;
      ...
      <td colspan="4" class="bg-light">
        <div class="border rounded p-2 bg-white">
          <div class="d-flex justify-content-between align-items-center mb-2">
            <div class="fw-semibold small">Sachnummern in ${escapeHtml(lgObj.lg)}</div>
            <div class="small text-muted">
              ${formatInt(lgObj.sachMap.size)} Sachnr. · ${formatInt(lgObj.paletten)} Paletten
            </div>
          </div>
  ...
  bindLgDrilldownEvents();

  if (totalEl) {
    totalEl.textContent =
      `${formatInt(totalPaletten)} Paletten · ${formatInt(totalStueck)} Stück · ${formatInt(totalSach)} Sachnummern`;
  }
}
...
function getSlotPositionData(slot) {
  const platzEl = slot.closest(".platz");

  const row = String(platzEl?.dataset.row || "?");
  const platz = String(platzEl?.dataset.platz || "?");
  const slotHuman = (parseInt(String(slot.dataset.slotIndex || "0"), 10) || 0) + 1;

  return {
    row,
    platz,
    slotHuman,
    ref: String(slot.dataset.ref || "-"),
    sach: String(slot.dataset.sach || "-"),
    menge: String(slot.dataset.menge || "1"),
    ls: String(slot.dataset.lieferschein || "-")
  };
}

function buildLgPositionButtonsHtml(slots) {
  const sorted = [...slots].sort((a, b) => {
    const pa = getSlotPositionData(a);
    const pb = getSlotPositionData(b);

    return (parseInt(pa.row, 10) || 0) - (parseInt(pb.row, 10) || 0)
      || (parseInt(pa.platz, 10) || 0) - (parseInt(pb.platz, 10) || 0)
      || (parseInt(pa.slotHuman, 10) || 0) - (parseInt(pb.slotHuman, 10) || 0);
  });

  const byRow = new Map();

  sorted.forEach((slot) => {
    const p = getSlotPositionData(slot);
    const row = String(p.row || "?");
    ...
    const rowObj = byRow.get(row);
    rowObj.paletten += 1;
    rowObj.stueck += Math.max(1, parseInt(String(slot.dataset.menge || "1"), 10) || 1);
    rowObj.slots.push(slot);
  });

  return [...byRow.values()]
    .sort((a, b) => (parseInt(a.row, 10) || 0) - (parseInt(b.row, 10) || 0))
    .map((rowObj) => {
      const rowKey = safeDomKey(`row_${rowObj.row}_${Date.now()}_${Math.random().toString(16).slice(2)}`);

      const slotButtonsHtml = rowObj.slots.map((slot, index) => {
        const p = getSlotPositionData(slot);

        const jumpId = `lgjump_${Date.now()}_${Math.random().toString(16).slice(2)}_${index}`;
        slot.dataset.lgJumpId = jumpId;

        return `
          <button type="button"
                  class="btn btn-sm btn-outline-secondary w-100 text-start mb-1 lg-position-btn"
                  data-lg-jump-id="${escapeHtml(jumpId)}">
            <span class="fw-semibold">
              P${escapeHtml(p.platz)} / Slot ${escapeHtml(p.slotHuman)}
            </span>
            <span class="text-muted">
              · Ref: ${escapeHtml(p.ref)}
              · Menge: ${escapeHtml(p.menge)}
            </span>
          </button>
        `;
      }).join("");
      ...
      return `
        <div class="border rounded mb-2 bg-white overflow-hidden">
          <button type="button"
                  class="btn btn-light w-100 text-start d-flex justify-content-between align-items-center lg-row-toggle"
                  data-lg-row-key="${escapeHtml(rowKey)}"
                  data-lg-row="${escapeHtml(rowObj.row)}">
            <span class="fw-semibold">
              <span class="me-1" data-lg-row-icon="${escapeHtml(rowKey)}">▸</span>
              Reihe ${escapeHtml(rowObj.row)}
            </span>

            <span class="small text-muted">
              ${formatInt(rowObj.paletten)} Paletten · ${formatInt(rowObj.stueck)} Stück
            </span>
          </button>

          <div class="d-none p-2 lg-row-slot-list"
               data-lg-row-detail="${escapeHtml(rowKey)}">
            ${slotButtonsHtml}
          </div>
        </div>
      `;
...
function bindLgDrilldownEvents() {
  const tbody = document.querySelector("#lgSummaryTable tbody");
  if (!tbody) return;

  tbody.querySelectorAll(".lg-drill-main").forEach(row => {
    row.addEventListener("click", () => {
      const lg = row.dataset.lg || "";
      const detail = tbody.querySelector(`tr[data-lg-detail="${cssEscape(lg)}"]`);
      const icon = row.querySelector("[data-lg-icon]");

      if (!detail) return;

      const willOpen = detail.classList.contains("d-none");

      tbody.querySelectorAll(".lg-drill-detail").forEach(el => el.classList.add("d-none"));
      tbody.querySelectorAll("[data-lg-icon]").forEach(el => el.textContent = "▸");
  ...
  tbody.querySelectorAll(".lg-sach-row").forEach(row => {
    row.addEventListener("click", (e) => {
      e.stopPropagation();

      const sachKey = row.dataset.sachKey || "";
      const detail = tbody.querySelector(`tr[data-sach-detail="${cssEscape(sachKey)}"]`);
      const icon = row.querySelector(`[data-sach-icon="${cssEscape(sachKey)}"]`);
      ...
      parentTable?.querySelectorAll(".lg-pos-row").forEach(el => el.classList.add("d-none"));
      parentTable?.querySelectorAll("[data-sach-icon]").forEach(el => el.textContent = "▸");
  ...
  tbody.querySelectorAll(".lg-row-toggle").forEach(btn => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();

      const rowKey = btn.dataset.lgRowKey || "";
      const rowNo = btn.dataset.lgRow || "";

      const detail = tbody.querySelector(`[data-lg-row-detail="${cssEscape(rowKey)}"]`);
      const icon = tbody.querySelector(`[data-lg-row-icon="${cssEscape(rowKey)}"]`);

      if (!detail) return;

      const willOpen = detail.classList.contains("d-none");

      const wrapper = btn.closest(".lg-position-list");
      wrapper?.querySelectorAll(".lg-row-slot-list").forEach(el => el.classList.add("d-none"));
      wrapper?.querySelectorAll("[data-lg-row-icon]").forEach(el => el.textContent = "▸");
      ...
      if (willOpen) {
        ...
        if (rowNo) {
          if (typeof showOnlySearchRow === "function") {
            showOnlySearchRow(rowNo);
          } else {
            showOnlyLgRow(rowNo);
          }

          setStatus?.(`Reihe ${rowNo} angezeigt.`, "info");
        }
      } else {
        detail.classList.add("d-none");
        if (icon) icon.textContent = "▸";
  ...
  tbody.querySelectorAll(".lg-position-btn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();

      const jumpId = btn.dataset.lgJumpId || "";
      const slot = document.querySelector(`.palette-slot[data-lg-jump-id="${cssEscape(jumpId)}"]`);

      if (!slot) {
        setStatus?.("Slot wurde nicht gefunden.", "error");
        return;
      }
      ...
      clearHighlights?.();
      slot.classList.add("ring-2", "ring-red-500");

      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (typeof jumpToSlot === "function") {
            jumpToSlot(slot, true);
          } else {
            slot.scrollIntoView({ behavior: "smooth", block: "center", inline: "center" });
            openSlotOverlay?.(slot);
          }
        });
      });

      setStatus?.(`Springe zu R${row}.`, "success");
    });
  });
}

function showOnlyLgRow(row) {
  const block = document.getElementById("w1-block-16-19");
  if (!block) return;

  const wanted = String(row || "").trim();
  if (!wanted) return;

  if (typeof window.ensureRowRendered === "function") {
    window.ensureRowRendered(wanted);
  } else if (typeof ensureRowRendered === "function") {
    ensureRowRendered(wanted);
  }

  block.querySelectorAll(".platz-container[data-row]").forEach(container => {
    const isWanted = String(container.dataset.row || "") === wanted;
    container.style.display = isWanted ? "" : "none";
  });

  block.querySelectorAll(".row-toolbar[data-row]").forEach(toolbar => {
    const isWanted = String(toolbar.dataset.row || "") === wanted;
    toolbar.style.display = isWanted ? "" : "none";
  });

  block.querySelectorAll(".lager-reihe[data-row]").forEach(rowEl => {
    const isWanted = String(rowEl.dataset.row || "") === wanted;
    rowEl.style.display = isWanted ? "" : "none";
  });
}
...
async function saveRowLabelToServer(row, label) {
  ...
  fd.append("halle", window.currentHall || "H3");
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    window.currentHall  = window.currentHall  || "H3";
    ...
    Halle3RowsLayout.build({
      ...
      bands: buildDynamicBands(getPlanRowFrom(), getPlanRowTo(), 20),
      placeMaxForRow
    });

    const preloadRows = [];
    for (let r = getPlanRowFrom(); r <= Math.min(getPlanRowFrom() + 5, getPlanRowTo()); r++) {
      preloadRows.push(r);
    }
    preloadRows.forEach(ensureRowRendered);
    ...
    console.error(e);
    alert("Lagerplan-Konfiguration konnte nicht geladen werden: " + (e?.message || e));
  }
});
...
window.__LAGER_CFG__ = window.__LAGER_CFG__ || null;

function defaultLagerCfg() {
  return {
    halle: "H3",
    zone: "W1",
    row_from: 1,
    ...
    row_overrides: {
      "20": { places: 25, slots_per_place: 20 },
      "43": { places: 35, slots_per_place: 4 }
    }
  };
}
...
async function loadRowConfigFromServer() {
  ...
  try {
    ...
    console.error("❌ lager_config_get.php liefert kein JSON:", text);
    throw new Error("Lager-Konfiguration ist kein gültiges JSON.");
  }
  ...
  window.__LAGER_CFG__ = js; // ✅ wichtig

  console.log("✅ Lager-Konfiguration geladen:", {
    halle: js.halle,
    zone: js.zone,
    row_from: ROW_FROM,
    row_to: ROW_TO,
    default_places_per_row: js.default_places_per_row,
    default_slots_per_place: js.default_slots_per_place,
    row_overrides: js.row_overrides || {}
  });
...
function getPlanCfg() {
  return window.__LAGER_CFG__ || defaultLagerCfg();
}

function getPlanRowFrom() {
  return Math.max(1, parseInt(getPlanCfg().row_from ?? 1, 10) || 1);
}

function getPlanRowTo() {
  const from = getPlanRowFrom();
  return Math.max(from, parseInt(getPlanCfg().row_to ?? 200, 10) || 200);
}

function getRowOverride(row) {
  const r = String(row ?? "").trim();
  return getPlanCfg().row_overrides?.[r] || null;
}

function buildDynamicBands(rowFrom, rowTo, chunkSize = 20) {
  const bands = [];

  for (let end = rowTo; end >= rowFrom; end -= chunkSize) {
    const start = Math.max(rowFrom, end - chunkSize + 1);
    bands.push({
      from: start,
      to: end,
      label: `Reihen ${start}–${end}`
    });
  }

  return bands;
}
function getRowLayoutCfg(row) {
  const cfg = window.__LAGER_CFG__ || {};
  const overrides = cfg.row_overrides || {};
  const key = String(parseInt(row, 10));
  ...
  return {
    places: Math.max(
      1,
      parseInt(ov?.places ?? cfg.default_places_per_row ?? 40, 10) || 40
    ),
    slotsPerPlace: Math.max(
      1,
      parseInt(ov?.slots_per_place ?? cfg.default_slots_per_place ?? 4, 10) || 4
    )
  };
}

function placeMaxForRow(row) {
  return getRowLayoutCfg(row).places;
}

function slotCapacityForRow(row) {
  return getRowLayoutCfg(row).slotsPerPlace;
}
...
function getGridColumnsForSlots(cap) {
  const n = Math.max(1, parseInt(cap, 10) || 1);
...
grid.style.gridTemplateColumns = `repeat(${getGridColumnsForSlots(cap)}, minmax(0, 1fr))`;
grid.style.gap = "2px";
...
function initManualEntry() {
  fillRowSelect("manualRow", getPlanRowFrom(), getPlanRowTo());
...
const verpackungRaw = (pack?.value || "").trim();
let verpackung = verpackungRaw !== "" ? verpackungRaw : null;
...
const allowedPackMap = new Map(
  allowedPacks.map(pack => [normalizePack(pack), pack])
);

if (verpackung !== null) {
  const normalizedPack = normalizePack(verpackung);

  if (!allowedPackMap.has(normalizedPack)) {
    setStatus("Ungültige Verpackung gewählt.", "error");
    return;
  }

  // offiziellen Wert verwenden
  verpackung = allowedPackMap.get(normalizedPack);
}
...
async function loadCorrectionsForPlan() {
  ...
  const urls = [
    `/Lagerplan/lager_corrections_api.php?action=list&halle=${encodeURIComponent(hall)}&zone=${encodeURIComponent(zone)}&batch_id=${encodeURIComponent(batchId)}`,
    `/Lagerplan/lager_corrections_api.php?action=list&halle=${encodeURIComponent(hall)}&zone=${encodeURIComponent(zone)}`,
    `/Lagerplan/lager_corrections_api.php?action=list`
  ];
  ...
  for (const u of urls) {
    try {
      console.log("🔎 Corrections lade:", u);
      ...
      console.log("🔎 Corrections Antwort:", {
        url: u,
        status: res.status,
      ...
      try {
        ...
        console.warn("❌ Corrections kein JSON:", u, text.slice(0, 500));
        continue;
      }

      if (res.ok && tmp && tmp.ok) {
        js = tmp;
        break;
      }

      console.warn("❌ Corrections JSON aber nicht ok:", u, tmp);

    } catch (err) {
      console.warn("❌ Corrections Fetch Fehler:", u, err);
    }
  }

  if (!js) {
    console.warn("Corrections: nichts geladen. Prüfe lager_corrections_api.php?action=list");
    return;
  }
  ...
  if (!items.length) {
    console.log("ℹ Corrections: keine Einträge vorhanden.");
    return;
  }
...
function ensureSlotSearchInfoModalDom() {
  let modal = document.getElementById("slotSearchInfoModal");
  if (modal) return modal;

  document.body.insertAdjacentHTML("beforeend", `
    <div id="slotSearchInfoModal"
         class="fixed inset-0 hidden items-center justify-center bg-black/40 z-[1000000]">
      ...
      <div class="bg-white rounded-xl shadow-xl p-4 w-[95vw] max-w-md">
        <div class="flex items-center justify-between mb-3">
          ...
          <button id="slotSearchInfoClose"
                  type="button"
                  class="text-slate-600 hover:text-slate-900 text-xl leading-none">
            ×
          </button>
        </div>
        ...
        <div class="grid gap-2 text-xs">
          <div class="grid grid-cols-3 gap-2">
            <div>
              ...
              <input id="ssiRow"
                     class="border border-slate-300 rounded px-2 py-1 w-full bg-slate-100"
                     readonly>
            </div>
          ...
          <div>
            ...
            <input id="ssiRef"
                   class="border border-slate-300 rounded px-2 py-2 w-full bg-slate-100 text-sm"
                   readonly>
          </div>
          ...
          <div class="grid grid-cols-2 gap-2">
            <div>
              <label class="block font-semibold">Lieferschein</label>
              <input id="ssiLs"
                     class="border border-slate-300 rounded px-2 py-2 w-full bg-slate-100 text-sm"
                     readonly>
            </div>
            ...
            <div>
              <label class="block font-semibold">Menge</label>
              <input id="ssiMenge"
                     class="border border-slate-300 rounded px-2 py-2 w-full bg-slate-100 text-sm"
                     readonly>
            </div>
          ...
          <div class="flex flex-wrap gap-2 mt-3">
            ...
            <button id="ssiEdit"
                    type="button"
                    class="bg-indigo-600 text-white text-xs font-semibold px-3 py-2 rounded">
              Bearbeiten
            </button>

            <button id="ssiFlag"
                    type="button"
                    class="bg-amber-600 text-white text-xs font-semibold px-3 py-2 rounded">
              Abweichung
            </button>

            <button id="ssiJump"
                    type="button"
                    class="bg-emerald-600 text-white text-xs font-semibold px-3 py-2 rounded">
              Zum Slot
            </button>

            <button id="ssiCancel"
                    type="button"
                    class="bg-slate-200 text-slate-800 text-xs font-semibold px-3 py-2 rounded">
              Schließen
            </button>
          </div>
        </div>
  ...
  modal = document.getElementById("slotSearchInfoModal");

  modal?.addEventListener("click", (e) => {
    if (e.target === modal) closeSlotSearchInfoModal();
  });

  document.getElementById("slotSearchInfoClose")?.addEventListener("click", closeSlotSearchInfoModal);
  document.getElementById("ssiCancel")?.addEventListener("click", closeSlotSearchInfoModal);

  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") closeSlotSearchInfoModal();
  });

  return modal;
}

function closeSlotSearchInfoModal() {
  const modal = document.getElementById("slotSearchInfoModal");
  if (!modal) return;
...
function openSlotSearchInfoModal(slot) {
  if (!slot) return;

  const modal = ensureSlotSearchInfoModalDom();
  if (!modal) return;

  const platzEl = slot.closest(".platz");
  if (!platzEl) return;

  // Kontext setzen, damit Bearbeiten/Karton/Abweichung denselben Slot kennen
  setOverlaySlotCtx?.(slot);

  window.__overlaySlotEl = slot;
  window.__overlaySlotId = String(slot.dataset.slotId || "");
  window.__overlayRef = String(slot.dataset.ref || "");
  ...
  document.getElementById("ssiRow").value = row;
  document.getElementById("ssiPlatz").value = platz;
  document.getElementById("ssiSlot").value = String(slotHuman);
  document.getElementById("ssiRef").value = ref;
  document.getElementById("ssiSach").value = sach;
  document.getElementById("ssiLs").value = ls;
  document.getElementById("ssiMenge").value = menge;
  document.getElementById("ssiDatum").value = datum;
  document.getElementById("ssiUser").value = user;

  const btnCartons = document.getElementById("ssiCartons");
  const btnEdit = document.getElementById("ssiEdit");
  const btnFlag = document.getElementById("ssiFlag");
  const btnJump = document.getElementById("ssiJump");

  btnCartons.onclick = () => {
    closeSlotSearchInfoModal();
    openCartonModal?.(slot);
  };
  ...
  btnEdit.onclick = () => {
    ...
    if (!slotId) {
      setStatus?.("Slot-ID fehlt. Bitte Seite neu laden und erneut suchen.", "error");
      soundError?.();
      return;
    }
  ...
  btnFlag.onclick = () => {
    closeSlotSearchInfoModal();
    window.Flagging?.openFlagModal?.(slot);
  };

  btnJump.onclick = () => {
    closeSlotSearchInfoModal();
    jumpToSlot?.(slot, false);
  };
...
const SCAN_SOUND_URL = "/Bilder/Sound/scann.mp3";
...
function soundError() {
  ...
  setTimeout(() => playScanSound(_sndErr, { rate: 0.85, volume: 0.9 }), 180);
}
...
function openPlaceMoveModal(platzEl) {
  ...
  if (pmRow && pmRow.options.length <= 1) {
    fillRowSelect("pmRow", getPlanRowFrom(), getPlanRowTo());
  }
...
function initMoveModal() {
  ...
  if (rowSel.options.length <= 1) {
    fillRowSelect("mmRow", getPlanRowFrom(), getPlanRowTo());
  }
...
async function searchQuery(queryRaw) {
  ...
  // Zum Slot springen, aber NICHT das kleine lager-info öffnen
  jumpToSlot(byRef, false);

  // Danach zentriertes Info-Fenster öffnen
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      openSlotSearchInfoModal(byRef);
    });
  });
...
// ===============================
function fillRowSelect(id, from = getPlanRowFrom(), to = getPlanRowTo()) {
...
function showHitList(title, slotList) {
  ...
  // Button oben: alle Reihen wieder anzeigen
  const topBar = document.createElement("div");
  topBar.className = "mb-2 flex items-center justify-between gap-2";

  topBar.innerHTML = `
    <button type="button"
      class="bg-slate-100 text-slate-700 border border-slate-300 text-[11px] font-semibold px-2 py-1 rounded hover:bg-slate-200"
      data-search-show-all-rows>
  ...
  topBar.querySelector("[data-search-show-all-rows]")?.addEventListener("click", () => {
    showAllSearchRows();
    clearHighlights();

    sorted.forEach(s => s.classList.add("ring-2", "ring-red-500"));
    setStatus("Alle Reihen werden wieder angezeigt.", "info");
  });

  // Nach Reihe gruppieren
  ...
  sorted.forEach((slotEl) => {
    ...
    grouped.get(row).push(slotEl);
  });

  // Accordion bauen
  [...grouped.entries()].forEach(([row, slots], index) => {
    const totalMenge = slots.reduce((sum, s) => {
      return sum + (parseInt(String(s.dataset.menge || "1"), 10) || 1);
    }, 0);

    const acc = document.createElement("div");
    acc.className = "border border-slate-200 rounded-lg overflow-hidden mb-2 bg-white";

    acc.innerHTML = `
      <button type="button"
        class="w-full px-3 py-2 bg-slate-50 hover:bg-slate-100 flex items-center justify-between gap-2 text-left"
        data-row-accordion-head="${escapeHtml(row)}">

        <span class="text-xs font-semibold text-slate-800">
          Reihe ${escapeHtml(row)}
        </span>

        <span class="text-[11px] text-slate-500">
          ${slots.length} Treffer · Menge ${totalMenge}
        </span>
      </button>

      <div class="${index === 0 ? "" : "hidden"} px-2 py-2 bg-white"
           data-row-accordion-body="${escapeHtml(row)}">
      </div>
    `;

    const head = acc.querySelector("[data-row-accordion-head]");
    const accBody = acc.querySelector("[data-row-accordion-body]");
    ...
    if (accBody) {
      slots.forEach((slotEl) => {
        ...
        const ls = slotEl.dataset.lieferschein || "";
        const menge = slotEl.dataset.menge || "1";

        const itemBtn = document.createElement("button");
        itemBtn.type = "button";
        itemBtn.className =
          "w-full text-left px-2 py-2 rounded-lg border border-slate-200 hover:bg-emerald-50 text-xs flex items-start justify-between gap-2 mb-2";

        itemBtn.innerHTML = `
          <span>
            <span class="font-semibold text-slate-800">
              R${escapeHtml(row)} / P${escapeHtml(platz)} / Slot ${escapeHtml(slotHuman)}
            </span><br>

            <span class="text-slate-500">
              Sach: ${escapeHtml(sach || "-")}
              ${ls ? " · LS: " + escapeHtml(ls) : ""}
              · Menge: ${escapeHtml(menge)}
            </span>
          </span>
        ...
        itemBtn.addEventListener("click", () => {
          ...
          requestAnimationFrame(() => {
            jumpToSlot(slotEl, true);
          });
        });

        accBody.appendChild(itemBtn);
      });
    }

    head?.addEventListener("click", () => {
      // alle Accordion-Bodies schließen
      bodyEl.querySelectorAll("[data-row-accordion-body]").forEach(el => {
        if (el !== accBody) el.classList.add("hidden");
      });

      // diese Reihe öffnen/schließen
      accBody?.classList.toggle("hidden");

      // nur diese Reihe im Plan anzeigen
      showOnlySearchRow(row);

      // Treffer in dieser Reihe markieren
      clearHighlights();
      ...
      if (firstSlot) {
        requestAnimationFrame(() => {
          jumpToSlot(firstSlot, false);
        });
      }

      setStatus(`Reihe ${row}: ${slots.length} Treffer angezeigt.`, "info");
    });
...
function showOnlySearchRow(row) {
  const block = document.getElementById("w1-block-16-19");
  if (!block) return;
  ...
  // Falls Lazy-Rendering aktiv ist
  if (typeof window.ensureRowRendered === "function") {
    window.ensureRowRendered(wanted);
  } else if (typeof ensureRowRendered === "function") {
    ensureRowRendered(wanted);
  ...
  // Reihen-Wrapper filtern, falls vorhanden
  block.querySelectorAll(".lager-reihe[data-row]").forEach(rowEl => {
    const isWanted = String(rowEl.dataset.row || "") === wanted;
    rowEl.style.display = isWanted ? "" : "none";
  });

  // Mobile Select mitziehen, falls vorhanden
  const mobileSel = document.getElementById("mobileRowSel");
  if (mobileSel) mobileSel.value = wanted;
}

function showAllSearchRows() {
  const block = document.getElementById("w1-block-16-19");
  if (!block) return;
  ...
  if (!slotEl) {
    try {
      const st = await fetchJson(
        `lager_ref_status.php?halle=${encodeURIComponent(window.currentHall || "H3")}` +
...
(function () {
  ...
  setTimeout(() => window.syncXlsxRowSelect?.(), 500);
    ...
    if (btnAll) btnAll.addEventListener('click', function(){
      ...
      downloadXlsx(rows, 'lagerplan_h3_gesamt_' + new Date().toISOString().slice(0,10) + '.xlsx', 'Gesamt');
    });

    var btnPerRow = document.getElementById('btnXlsxPerRow');
    if (btnPerRow) btnPerRow.addEventListener('click', function(){
      var rows = collectRows();
      downloadXlsxPerRow(rows, 'lagerplan_h3_reihen_' + new Date().toISOString().slice(0,10) + '.xlsx');
    });
    ...
    if (btnRow) btnRow.addEventListener('click', function(){
      ...
      downloadXlsx(rows, 'lagerplan_h3_reihe_' + r + '_' + new Date().toISOString().slice(0,10) + '.xlsx', 'Reihe ' + r);
    });
  });
})();
...
function bindEditModalHandlers() {
  ...
  if (oldBtn.dataset.h3SaveBound === "1") return;

  const freshBtn = oldBtn.cloneNode(true);
  oldBtn.parentNode.replaceChild(freshBtn, oldBtn);
  freshBtn.dataset.h3SaveBound = "1";
...
const res = await fetch("/Lagerplan/lager_corrections_api.php?action=upsert", {
  method: "POST",
...
async function updateRefOnServer(slotId, newRef) {
  ...
  const res = await fetch("/Lagerplan/lager_ref_update.php", {
    method: "POST",
    body: fd,
    credentials: "include",
    cache: "no-store"
...
function buildRowPlaces(container) {
  ...
  for (let p = start; p <= end; p++) {
    ...
    for (let i = 0; i < cap; i++) {
      ...
      slot.className = "palette-slot";
      slot.dataset.slotIndex = String(i);
      slot.dataset.slotLabel = `Slot ${i + 1}`;
      grid.appendChild(slot);
    }
...
(function initInventoryOnce(){
  ...
  const invKey = () => `inv_checked_${window.currentHall || "H3"}_${window.currentZone || "W1"}`;
...
function exportPalletsPerRowXlsx() {
  ...
  const hall = window.currentHall || "H3";
  const zone = window.currentZone || "W1";
  const fromRow = getPlanRowFrom();
const toRow   = getPlanRowTo();
...
// Erwartetes Datenformat pro Datensatz:
// {
//   reihe: "H3-R04",         // oder reiheNum: 4 -> wir machen daraus H3-R04
//   position: 1,
//   sachnummer: "05E 117 021 C",
//   sys_qty: 0, sys_cnt: 0,  // optional (Systembestand)
//   ist_qty: 56, ist_cnt: 10 // F und G
// }
...
(() => {
  ...
  const toRowName = (x) => {
    if (typeof x === "string" && x.startsWith("H3-R")) return x;
    const n = Number(x);
    return `H3-R${String(n).padStart(2, "0")}`; // 1->H3-R01, 140->H3-R140
  };

  const getData = () => {
    const data = window.h3ExportData;
    if (!Array.isArray(data)) {
      alert("❌ window.h3ExportData fehlt (Export-Daten nicht vorhanden).");
      return null;
    }
    // reihe normalisieren
    return data.map(d => ({
...
const TEMPLATE_URL = "/Lagerplan/templates/Lagerplatzkontrolle_Vorlage.xlsx";
...
async function exportInventurExcel(slots) {
  ...
  // 2) Jede Reihe in passendes Tabellenblatt schreiben (H3-R04 usw.)
  for (const [rowNoStr, items] of Object.entries(byRow)) {
    const rowNo = Number(rowNoStr);
    const sheetName = `H3-R${String(rowNo).padStart(2, "0")}`;
...
const filename = `Inventur_H3_${new Date().toISOString().slice(0, 10)}.xlsx`;
...
(function () {
  ...
  const url = `/Lagerplan/api/halle3_export_slots.php${qs}`;
