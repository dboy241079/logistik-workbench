...
function findRowForField(field){
  for (let i=0;i<dayRows.length;i++){
    ...
    if (field==='arriveH2' && r.departH && (!r.arriveH2) && r.hannoverHall2) return {i,r};
...
document.addEventListener('DOMContentLoaded', async () => {
  // 🟢 Fahrzeuge zuerst vom Server laden
  await loadVehicles();
  ...
  if (vehSelect && !vehSelect.value)
    vehSelect.value = vehSelect.options[0]?.value || 'veh1';
...
// ganz am Anfang, direkt nach window.API_BASE usw.
async function loadVehicles() {
  try {
    const res = await fetch(`${API_BASE}/veh_cfg.php`, { cache: 'no-store' });
    const data = await res.json();
    if (data.ok && data.cfg && Array.isArray(data.cfg.vehicles)) {
      VEHICLES = data.cfg.vehicles;
    } else {
      console.warn('veh_cfg: keine gültigen Fahrzeuge erhalten', data);
      VEHICLES = [];
    }
  } catch (err) {
...
function labelFor(field){
  return ({
  arriveWU:'Ankunft Wunstorf',  departWU:'Abfahrt Wunstorf',
  arriveH:'Ankunft Hannover',   departH:'Abfahrt Hannover',
  arriveH2:'Ankunft Hannover 2',departH2:'Abfahrt Hannover 2'
})[field] || field;
}

window.pickHall = function(initial = '') {
  return new Promise(resolve => {
    const modalEl = document.getElementById('hallModal');
    const selectEl = document.getElementById('hallSelect');
...
async function stampField(field){
  ...
  if (!t){ 
    setStatus(`Kein offener Eintrag für „${labelFor(field)}“ gefunden.`, false); 
    return; 
  }
  ...
  // 🟢 1. Wenn Ankunft Hannover → Halle auswählen
  if (field === 'arriveH'){
  ...
  // 🟢 2. Wenn Abfahrt Hannover → fragen, ob zweite Halle kommt
  if (confirmed){
  // zweite Halle auswählen
  ...
  // aktuelle Zeit als Start für Halle 2
  const now2 = hhmmNow();

  const payload2 = {
    departH: val,           // Abfahrt H1
    arriveH2: now2,         // Ankunft H2 sofort danach
    hannoverHall2: hall2    // gewählte Halle 2
  };

  await apiStamp(vehSelect.value, dateSelect.value, t.r.tour, payload2);

  setStatus(`Gespeichert: Abfahrt H1 = ${val} → Ankunft 2. Halle (${hall2}) = ${now2}`, true);
  await reloadDay();
  return;
...
(async function syncCfgFromServerOnce(){
  try {
    const res = await fetch((window.API_BASE||'/LKW/api') + '/veh_cfg.php', { credentials:'include', cache:'no-store' });
  ...
  var CFG = loadConfig();
  VEHICLES = deepClone(CFG.vehicles); // 🟢 ohne var!
...
function renderVeh(veh, mountId) {
  ...
  const rows = full.map((r, i) => ({ r, i })).filter(x => weekSet.has(x.r.date));
  const todayISO = isoDate(new Date());
  ...
  html.push('<th colspan="4" class="table-success text-center">Hannover 1</th>');
  html.push('<th rowspan="2" class="text-center">2. Halle?</th>');
  html.push('<th colspan="4" class="table-success text-center">Hannover 2</th>');
  html.push('<th colspan="3" class="table-warning text-center">Pause</th>');
  ...
  html.push('<th>Ankunft</th><th>Halle</th><th>Abfahrt</th><th>Dauer</th>');
  html.push('<th>Ankunft</th><th>Halle</th><th>Abfahrt</th><th>Dauer</th>');
  html.push('<th>Start</th><th>Ende</th><th>Dauer</th>');
  html.push('</tr>');
  html.push('</thead><tbody>');

  let currentDate = null, daySumWU = 0, daySumH = 0, daySumPause = 0;
  ...
  function flushDaySubtotal() {
    ...
    html.push(`
      <tr class="table-light fw-semibold">
        <td colspan="7" class="text-end">Tagessumme (${currentDate}):</td>
        <td>${daySumWU ? fmtHM(daySumWU) : '0:00'}</td>
        <td></td><td></td><td>${daySumH ? fmtHM(daySumH) : '0:00'}
        ${total ? '<small class="text-muted">(Gesamt: ' + fmtHM(total) + ')</small>' : ''}</td>
        <td>${daySumPause ? fmtHM(daySumPause) : '0:00'}</td>
        <td colspan="4"></td>
      </tr>`);
    daySumWU = daySumH = daySumPause = 0;
    currentDate = null;
  }

  for (const { r, i: ix } of rows) {
    const hallWU = dwellWu(r);
    const hallH1 = dwellHan1(r);
    const hallH2 = dwellHan2(r);
    const pauseDur = duration(r.pauseStart, r.pauseEnd);
    const st = statusLabel(r);
    const isToday = r.date === todayISO;

    if (currentDate === null) currentDate = r.date;
    if (r.date !== currentDate) { flushDaySubtotal(); currentDate = r.date; }

    daySumWU += hallWU || 0;
    daySumH += (hallH1 || 0) + (hallH2 || 0);
    daySumPause += pauseDur || 0;

    html.push(`<tr class="${isToday ? 'table-info' : ''}">
      <td>${r.tour}</td>
      <td><input type="date" class="form-control form-control-sm" value="${r.date}" data-veh="${veh.id}" data-row="${ix}" data-field="date"></td>
      <td class="text-muted small">${wday(r.date)}</td>
      <td><input type="text" class="form-control form-control-sm time-input" value="${r.workStart || ''}" data-veh="${veh.id}" data-row="${ix}" data-field="workStart"></td>

      <!-- Wunstorf -->
      <td><input type="text" class="form-control form-control-sm time-input" value="${r.arriveWU || ''}" data-veh="${veh.id}" data-row="${ix}" data-field="arriveWU"></td>
      <td><input type="text" class="form-control form-control-sm time-input" value="${r.departWU || ''}" data-veh="${veh.id}" data-row="${ix}" data-field="departWU"></td>
      <td>${showHM(hallWU)}</td>

      <!-- Hannover 1 -->
      <td><input type="text" class="form-control form-control-sm time-input" value="${r.arriveH || ''}" data-veh="${veh.id}" data-row="${ix}" data-field="arriveH"></td>
      <td>
        <select class="form-select form-select-sm" data-veh="${veh.id}" data-row="${ix}" data-field="hannoverHall">
          <option value="" ${!r.hannoverHall ? 'selected' : ''}>–</option>
          <option value="28" ${r.hannoverHall === '28' ? 'selected' : ''}>28</option>
          <option value="28C" ${r.hannoverHall === '28C' ? 'selected' : ''}>28C</option>
          <option value="34" ${r.hannoverHall === '34' ? 'selected' : ''}>34</option>
        </select>
      </td>
      <td><input type="text" class="form-control form-control-sm time-input" value="${r.departH || ''}" data-veh="${veh.id}" data-row="${ix}" data-field="departH"></td>
      <td>${showHM(hallH1)}</td>

      <!-- Button zweite Halle -->
      <td class="text-center">
        <button class="btn btn-sm btn-outline-secondary" data-veh="${veh.id}" data-row="${ix}" data-action="askH2" title="Weitere Halle?">
          <i class="bi bi-arrow-right-circle"></i>
        </button>
      </td>

      <!-- Hannover 2 -->
      <td><input type="text" class="form-control form-control-sm time-input" value="${r.arriveH2 || ''}" data-veh="${veh.id}" data-row="${ix}" data-field="arriveH2"></td>
      <td>
        <select class="form-select form-select-sm" data-veh="${veh.id}" data-row="${ix}" data-field="hannoverHall2">
          <option value="" ${!r.hannoverHall2 ? 'selected' : ''}>–</option>
          <option value="28" ${r.hannoverHall2 === '28' ? 'selected' : ''}>28</option>
          <option value="28C" ${r.hannoverHall2 === '28C' ? 'selected' : ''}>28C</option>
          <option value="34" ${r.hannoverHall2 === '34' ? 'selected' : ''}>34</option>
        </select>
      </td>
      <td><input type="text" class="form-control form-control-sm time-input" value="${r.departH2 || ''}" data-veh="${veh.id}" data-row="${ix}" data-field="departH2"></td>
      <td>${showHM(hallH2)}</td>

      <!-- Pause -->
      <td><input type="text" class="form-control form-control-sm time-input" value="${r.pauseStart || ''}" data-veh="${veh.id}" data-row="${ix}" data-field="pauseStart"></td>
      <td><input type="text" class="form-control form-control-sm time-input" value="${r.pauseEnd || ''}" data-veh="${veh.id}" data-row="${ix}" data-field="pauseEnd"></td>
      <td>${showHM(pauseDur)}</td>

      <!-- Status -->
      <td><span class="badge bg-${
        st === "Parkplatz" ? "secondary" :
        st.startsWith("In Halle Hannover") ? "warning" :
        st === "Nach Hannover" || st === "Auf dem Weg nach Wunstorf" ? "primary" :
        st === "Feierabend" ? "success" : "secondary"
      }">${st}</span></td>
      ...
      <!-- Meldung -->
      <td class="d-flex flex-column gap-1" style="min-width:260px;">
        <select class="form-select form-select-sm" data-veh="${veh.id}" data-row="${ix}" data-field="reported">
        ...
        <textarea class="form-control form-control-sm" rows="2" placeholder="Warum?" data-veh="${veh.id}" data-row="${ix}" data-field="reportedWhy"
          style="${r.reported === 'Nein' ? '' : 'display:none'}">${r.reportedWhy || ''}</textarea>
      </td>
    </tr>`);
  }
  ...
  // === Änderungen speichern & API sync ===
  const inputs = mount.querySelectorAll('input,select,textarea');
  inputs.forEach(inp => {
    inp.addEventListener('change', ev => {
      const v = ev.target.dataset.veh;
      const idx = +ev.target.dataset.row;
      ...
      if (f === 'reported') {
        ...
        const whyInput = cell?.querySelector('textarea[data-field="reportedWhy"]');
  ...
  // === Button "2. Halle?" Handler ===
  mount.querySelectorAll('button[data-action="askH2"]').forEach(btn => {
    btn.addEventListener('click', async () => {
      const vehId = btn.dataset.veh;
      const idx = +btn.dataset.row;
      const r = DRV.state.data[vehId][idx];
      const confirmed = confirm("Fährst du zu einer weiteren Halle in Hannover?");
      if (!confirmed) return;
      const hall2 = await pickHall(r.hannoverHall2 || '');
      if (!hall2) return;
      const now = hhmmNow();
      await apiStamp(vehId, r.date, r.tour, {
        departH: r.departH || now,
        arriveH2: now,
        hannoverHall2: hall2
      });
...
function renderVehDay(vehId, dateISO) {
  const arr = (DRV.state.data[vehId] || []).filter(r => r.date === dateISO);
  let html = `
    <table class="table table-sm table-bordered align-middle">
      <thead>
        <tr>
          <th>Tour</th><th>Start</th>
          <th>WU an</th><th>WU ab</th>
          <th>H an</th><th>H ab</th>
          <th>Pause Start</th><th>Pause Ende</th>
          <th>Ende</th>
        </tr>
      </thead>
      <tbody>
  ...
  html += '</tbody></table>';
  document.getElementById(`${vehId}Wrap`).innerHTML = html;
}
// === Dauerhelfer ===

// Wunstorf-Dauer (Ankunft → Abfahrt)
function dwellWu(r){
  return duration(r.arriveWU, r.departWU);
}

// Hannover-Dauer (H1)
function dwellHan1(r){
  return duration(r.arriveH, r.departH);
}

// Hannover-Dauer (H2)
function dwellHan2(r){
  return duration(r.arriveH2, r.departH2);
}

// Gesamtdauer Hannover (H1 + H2)
function dwellHan(r){
  const d1 = dwellHan1(r) || 0;
  const d2 = dwellHan2(r) || 0;
  return (d1 + d2) || null;
}
...
function renderOverview() {
  ...
  const container = document.getElementById('drvOverviewWrap');
  if (!container) {
    console.warn('renderOverview: #drvOverviewWrap nicht gefunden.');
    return;
  }
  ...
  const html = [];
  html.push(`<table class="table table-hover align-middle table-bordered small">`);
  html.push(`
    <thead class="table-dark text-center">
      <tr>
        ...
        <th>Tour</th>
        <th>Start</th>
        <th>Ankunft Wunstorf</th>
        <th>Abfahrt Wunstorf</th>
        <th>Dauer WU</th>
        <th>Ankunft Hannover 1</th>
        <th>Halle 1</th>
        <th>Abfahrt Hannover 1</th>
        <th>Dauer H1</th>
        <th>Ankunft Hannover 2</th>
        <th>Halle 2</th>
        <th>Abfahrt Hannover 2</th>
        <th>Dauer H2</th>
        <th>Pause Start</th>
        <th>Pause Ende</th>
        <th>Pause Dauer</th>
  ...
  for (const dateISO of weekDates) {
    ...
    for (const veh of VEHICLES) {
      const vehRows = (DRV.state.data[veh.id] || []).filter(r => r.date === dateISO);
      if (vehRows.length) dayRows.push({ veh, rows: vehRows });
    }
    if (dayRows.length === 0) continue;

    html.push(`
      <tr class="table-primary fw-bold">
        <td colspan="22">${new Date(dateISO).toLocaleDateString('de-DE')} – ${wday(dateISO)}</td>
      </tr>
    `);

    for (const group of dayRows) {
      const { veh, rows } = group;
      html.push(`
        <tr class="table-secondary">
          <td colspan="22"><i class="bi bi-truck"></i> ${veh.name || veh.id}</td>
        </tr>
      `);

      let totalWU = 0, totalH1 = 0, totalH2 = 0, totalPause = 0;

      for (const r of rows) {
        const wu = dwellWu(r);
        const h1 = dwellHan(r);
        const h2 = dwellHan2(r);
        const pause = duration(r.pauseStart, r.pauseEnd);
        totalWU += wu || 0;
        totalH1 += h1 || 0;
        totalH2 += h2 || 0;
        totalPause += pause || 0;
        ...
        const badge =
          (r.status === "Parkplatz") ? "secondary" :
          (r.status?.includes("Hannover") ? "success" :
          (r.status?.includes("Wunstorf") ? "primary" : "warning"));

        html.push(`
          <tr ${r.date === todayISO ? 'class="table-info"' : ''}>
            <td>${r.date}</td>
            <td>${wday(r.date)}</td>
            <td>${veh.name || veh.id}</td>
            <td>${r.tour}</td>
            <td>${r.workStart || ''}</td>
            <td>${r.arriveWU || ''}</td>
            <td>${r.departWU || ''}</td>
            <td>${fmtHM(wu)}</td>
            <td>${r.arriveH || ''}</td>
            <td>${r.hannoverHall || ''}</td>
            <td>${r.departH || ''}</td>
            <td>${fmtHM(h1)}</td>
            <td>${r.arriveH2 || ''}</td>
            <td>${r.hannoverHall2 || ''}</td>
            <td>${r.departH2 || ''}</td>
            <td>${fmtHM(h2)}</td>
            <td>${r.pauseStart || ''}</td>
            <td>${r.pauseEnd || ''}</td>
            <td>${fmtHM(pause)}</td>
            <td><span class="badge bg-${badge}">${statusLabel(r)}</span></td>
            <td>${r.note || ''}</td>
            <td>${r.reported || ''}</td>
          </tr>
        `);
      ...
      html.push(`
        <tr class="table-light fw-semibold text-end">
          <td colspan="7">Summe ${veh.name || veh.id}:</td>
          <td>${fmtHM(totalWU)}</td>
          <td colspan="3"></td>
          <td>${fmtHM(totalH1)}</td>
          <td colspan="3"></td>
  ...
  // ✅ hier ist container garantiert vorhanden
  ...
  function dwellHan1(r){ return duration(r.arriveH,  r.departH); }
  function dwellHan2(r){ return duration(r.arriveH2, r.departH2); }
  function dwellHan(r){  // bestehende Funktion überschreiben/ersetzen
  return (dwellHan1(r)||0) + (dwellHan2(r)||0) || null;
}
...
function applyServerDay(vehId, dateISO, serverRows){
  ...
  for (let t=1; t<=TOURS_PER_DAY; t++){
    ...
    c.arriveH2      = s.arriveH2      || '';
    c.departH2      = s.departH2      || '';
    c.hannoverHall2 = s.hannoverHall2 || '';
...
if (driversTabBtn){
  ...
  if (r.departH && r.hannoverHall2 && !r.arriveH2) return "Nach Hannover (2. Halle)";
  if (r.arriveH2 && !r.departH2) return "In Halle Hannover 2" + (r.hannoverHall2 ? " ("+r.hannoverHall2+")" : "");
  if (r.departH2 && !r.arriveWU) return "Auf dem Weg nach Wunstorf";

  if (r.departH && !r.arriveWU && !r.hannoverHall2) return "Auf dem Weg nach Wunstorf";
  ...
  if (r.workEnd) return "Feierabend";
  return "Parkplatz";
}
