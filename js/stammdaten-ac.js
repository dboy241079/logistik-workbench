(function () {
  ...
  // ===== Globaler Guard: Script nur EINMAL initialisieren ====================
  if (window.__STMD_AC_V2_LOADED__) {
    console.warn('stammdaten-ac.js: zweite Einbindung erkannt – init wird übersprungen');
    return;
  }
  window.__STMD_AC_V2_LOADED__ = true;
  ...
  // KEIN datalist mehr -> kein inputEl.setAttribute("list", ...)
  if (!inputEl) return;

  // einmal laden anstoßen
  ensureLoaded().catch(() => {});
  ...
  // Pick erst bei change/blur (damit es sauber ist)
  inputEl.addEventListener('change', handlePick);
  inputEl.addEventListener('blur', handlePick);
}
  ...
  function autoBindForTable(tableId, opts = {}) {
    ...
    // 💡 WICHTIG: Guard, damit die Tabelle nur EINMAL gebunden wird
    if (table.dataset._acBound === '1') {
      // console.debug('stammdaten-ac: Tabelle bereits gebunden:', tableId);
      return;
    }
    table.dataset._acBound = '1';
