...
try {
  // Rollen prüfen (anpassen nach Bedarf)
  ...
  // Zielordner
  $dirAbs = dirname(__DIR__) . '/uploads/signatures/prepared';
  if (!is_dir($dirAbs) && !mkdir($dirAbs, 0775, true) && !is_dir($dirAbs)) {
    throw new RuntimeException('Konnte Signaturordner nicht erstellen.');
  }

  $fileName = sprintf(
    'prepared_order_%d_%s_%s.png',
    $orderId,
    preg_replace('/[^a-zA-Z0-9_-]/', '_', $username ?: 'unknown'),
    date('Ymd_His')
  );

  $fileAbs = $dirAbs . '/' . $fileName;
  $fileRel = '/uploads/signatures/prepared/' . $fileName;
  ...
  // Auftrag prüfen
  $st = $pdo->prepare("SELECT id, status FROM kommi_orders WHERE id = ? LIMIT 1");
  $st->execute([$orderId]);
  ...
  // Status + Signatur speichern
  // -> Passe status-Feld / Werte an dein System an
  $st = $pdo->prepare("
  UPDATE kommi_orders
  SET
