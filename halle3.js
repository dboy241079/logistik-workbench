...
async function loadSachStammMap() {
  ...
  if (!res.ok || !j.ok || !Array.isArray(j.items)) throw new Error(j?.error || "stammdaten_list_failed");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  [1, 20, 26, 51, 76, 101].forEach(ensureRowRendered);
...
function placeMaxForRow(row) {
  ...
  if (!Number.isFinite(r)) return 35;
  if (r === 20) return 25;
  return 35;
}
...
setTimeout(() => window.syncXlsxRowSelect?.(), 200);
  ...
  btnSave?.addEventListener("click", async () => {
    ...
    const dupClient = document.querySelector(`#w1-block-16-19 .palette-slot[data-ref="${cssEscape(referenznr)}"]`);
    if (dupClient) {
    ...
    let platzEl = document.querySelector(`#w1-block-16-19 .platz[data-row="${reihe}"][data-platz="${platzNr}"]`);
    ...
    if (!free) {
      ...
      if (suggestion?.platzEl && suggestion?.slotEl) {
        ...
        setStatus(`Platz voll → weiter auf R${reihe} / P${String(suggestion.platz).padStart(2, "0")}`, "error");

        platzEl = suggestion.platzEl;
        free = suggestion.slotEl;
      } else {
        const cap = platzEl.querySelectorAll(".palette-slot").length;
        setStatus(`Dieser Platz ist voll (${cap}/${cap}) und kein weiterer freier Platz in der Reihe gefunden.`, "error");
        return;
      }
    }
    ...
    try {
      const data = await saveSlotToServer({
        ...
        menge: menge                 // ✅ WICHTIG: wird jetzt wirklich gesendet
      });
  ...
  sachnummer, lieferschein, referenznr, reihe, platzRaw, menge
});
  });
  ...
  fetchJson(url)
    .then(rows => {
      ...
      rows.forEach(row => {
        ...
        if (row.id) slot.dataset.slotId = String(row.id);
...
async function fetchJson(url, opts = {}) {
  ...
  try {
    ...
    console.error("❌ Netzwerkfehler bei fetch:", url, netErr);
    throw netErr;
  }

  const text = await res.text();

  // Immer loggen, wenn HTTP nicht OK
  if (!res.ok) {
    ...
    // Versuche JSON-Fehler sauber zu lesen
    try {
  ...
  try {
    ...
    console.error("❌ Server liefert kein JSON:", {
      url,
      status: res.status,
...
function applySlotToUI(slotEl, { ref, sach, lieferschein, user, menge }) {
  ...
  slotEl.title = `${ref} · ${sach} · ${todayDE}${mTxt}`;
}
...
function injectRowPrintButtons() {
  ...
  document.querySelectorAll('.platz-container[data-row]').forEach(pc => {
    ...
    // schon vorhanden?
    if (pc.previousElementSibling?.classList?.contains("row-toolbar") &&
        pc.previousElementSibling?.dataset?.row === row) {
      return;
    }
    if (document.querySelector(`.row-toolbar[data-row="${cssEscape(row)}"]`)) return;
   ...
   toolbar.innerHTML = `
  <div class="fw-semibold" style="font-size:11px;color:#334155;">${rowDisplay(row)}</div>
...
`;
    ...
    // ✅ direkt vor den Container setzen (nicht in block.firstChild)
...
function bindEditModalHandlers() {
  ...
  return;
    ...
    const payload = { slot_id: Number(slotId), sach_korr, qty_korr, note };
...
function buildRowPlaces(container) {
  ...
  const row   = container.dataset.row;
  const start = parseInt(container.dataset.rangeStart, 10) || 1;
  const end   = parseInt(container.dataset.rangeEnd, 10)   || placeMaxForRow(row);
...
(function bindEditButtonDelegatedOnce(){
  ...
  document.addEventListener("click", (e) => {
          // ✅ Primär: globaler Kontext
      let slotEl = window.__overlaySlotEl || null;

      // ✅ Fallback: aus #lager-info Slot-ID lesen und Slot im DOM finden
      if (!slotEl) {
        const info = document.getElementById("lager-info");
      ...
      if (!slotEl) {
        throw new Error("Kein Slot-Kontext. Öffne erst ein Slot-Overlay (Ref anklicken).");
      }
    ...
    try {
      const slotEl = window.__overlaySlotEl;
      if (!slotEl) throw new Error("Kein Slot-Kontext (Overlay). Öffne erst ein Slot-Overlay.");
      ...
      // Slot-ID MUSS da sein, sonst kann Server nix updaten
      const slotId = slotEl.dataset.slotId || "";
      if (!slotId) throw new Error("Slot-ID fehlt (dataset.slotId). Bitte einmal neu laden.");

      // Debug (solltest du im Log sehen)
      ...
      console.error("❌ Bearbeiten failed:", err);
      setStatus?.(("Bearbeiten: " + (err?.message || err)), "error");
      soundError?.();
    }
  }, false);
})();
