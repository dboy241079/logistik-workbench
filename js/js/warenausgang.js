...
function applyPalletRunningBadges() {
  ...
  while (i < rows.length) {
    ...
    if (key !== '') {
      ...
      const sumPal = group.reduce((acc, r) => acc + toInt(getCellValue(r, 10)), 0); // Spalte 10 = Behälter

      // Badge nur in der ERSTEN Zeile der Gruppe, in der Behälter-Zelle
      const td = group[0]?.children[10];
      if (td) {
...
function applyGroupBadges() {
  const COL_BEH = 10, COL_ZUS = 11, COL_KG = 14, COL_GEB = 15;
  ...
  while (i < rows.length) {
    ...
    if (key !== '') {
      ...
      const kgSum  = group.reduce((a, r) => a + toInt(getCellValue(r, COL_KG)),  0);
      ...
      // --- Paletten-Badge (zwischen Behälter und Zus.Beh) -------------------
      {
        ...
        placeBetweenCellsBadge(first, COL_BEH, COL_ZUS, bPal);
      }
...
(async () => {
  try {
    const res = await fetch('/LKW/api/stammdaten_api.php?type=behaelter&action=list', { credentials:'same-origin' });
    const j = await res.json();
...
function computeStats() {
  ...
  // gewünschte Reihenfolge zuerst
  const TARGET_GROUPS = ["W1", "X3", "X3(B)", "G9", "B1", "Bauteile", "BM", "Müll"];

  // --- Aggregations-Container ---
  const sumBySach = new Map();                         // Sachnr. -> Summe Behälter
  const offeneSet = new Set();                         // offene ausgangsnummern (distinct)
  const offeneList = new Map();                        // für Ausgabe

  // WICHTIG: dynamisch! (nicht nur TARGET_GROUPS)
  const sumByLagergruppe = new Map();                  // LG -> Summe Behälter
  const detailsByLG = new Map();                       // LG -> { deliveries:Set, zus, brtGew, perAusgang:Map }

  // Zeilenbasis (gespeichert oder edit)
  ...
  const perDayMap = new Map(); // 'YYYY-MM-DD' -> Set<ausgangsnummer>
...
if (range.end && (!dt || dt >= range.end)) return;
    ...
    const brtGew    = toInt(getCellValue(tr, 14));
    const gebucht   = ((getCellValue(tr, 15) || '').trim() !== '');
    const ausgang   = (getCellValue(tr, 0) || '').trim();
    const lgRaw     = (getCellValue(tr, 2) || '').trim();
    const lg        = lgRaw; // kann leer sein -> '' als Key
    ...
    // --- DYNAMISCHE LG-AGGREGATION ---
    if (!detailsByLG.has(lg)) {
      detailsByLG.set(lg, { deliveries:new Set(), zus:0, brtGew:0, perAusgang:new Map() });
    }
...
bindSachStatsInteractions?.();
  ...
  // ===== Lagergruppen-Karte (jetzt inkl. dynamischer Gruppen) =====
  ...
  if (tbodyGruppen && gesamtEl) {
    ...
    order.forEach(grp => {
      ...
      const tr = document.createElement("tr");
      tr.dataset.grp = grp; // WICHTIG: Key bleibt original ('' möglich)
      tr.innerHTML = `
        <td>
...
function makeActionGroup(editMode) {
  ...
  btnEdit.addEventListener("click", async (e) => {
    ...
    if (isEditing) {
      ...
      window.__bindRowAC = (tr) => window.StammdatenAC?.bindRowAC(tr, {
...
function editRow(tr) {
  ...
  // <<< NEU: LG live aus Sachnummer ODER Behälternummer ermitteln >>>
  // LG live aus Behälternummer ODER Sachnummer ermitteln
const behnrInp = tr.children[12]?.querySelector('input');
const sachInp  = tr.children[13]?.querySelector('input');
const reapplyLG = () => applyLGForRow(tr);
behnrInp?.addEventListener('input', reapplyLG);
...
function saveRow(tr) {
  ...
  // Lagergruppe gemäß Priorität setzen (Sachnummer > Behälternummer > leer)
...
btnResetFilter?.addEventListener("click", () => {
  ...
  if (!filterSelect) return; // kein Dropdown vorhanden → nichts zu tun
...
searchField?.addEventListener("change", () => {
  ...
  const selected = filterSelect?.value || "";                 // kann fehlen
  const query    = (searchInput?.value || "").trim();         // kann fehlen
  const fieldKey = searchField?.value || "ausgang";           // fallback

  Array.from(tbody.querySelectorAll("tr")).forEach((tr) => {
    const key = getCellValue(tr, 0);
    const matchDropdown = !selected || key === selected;      // wenn kein Dropdown → immer true
...
async function renderAttachmentList(ausgangNr) {
  ...
  if (keepUrl) {
    const stillThere = files.some(x => `/LKW/${x.path_rel}` === keepUrl);
